...
from spacy.util import get_base_version, get_lang_class
...
def main(
    # fmt: off
    spacy_version: str = typer.Argument(..., help="The spaCy version"),
    spacy_streamlit_version: str = typer.Argument(..., help="The version of spacy-streamlit"),
    ...
    r.raise_for_status()
    compat = r.json()["spacy"]
    base_version = get_base_version(spacy_version)
    if base_version not in compat:
        raise ValueError(f"No compatible models found for {base_version} ({spacy_version})")
    data = compat[base_version]
    reqs = [
